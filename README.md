...
一站式自动化测试平台，支持**接口测试**、**Web 自动化测试**、**AI 智能生成用例**。具备用例管理、批量执行、异步任务、可视化报告等企业级特性。
...
NetCatch 专为个人开发者与小型团队设计，帮助快速构建自动化测试体系，降低重复劳动。
...
```bash
# 克隆项目
git clone https://github.com/yourname/netcatch.git
cd netcatch
...
服务端口：
- 前端 + 后端：`http://localhost:5000`
- Redis：`127.0.0.1:6379`（容器内部）
